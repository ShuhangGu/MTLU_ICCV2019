...
class DIV2KSR(ImageData.ImageData):
    def __init__(self, args, train=True):
        super(DIV2KSR, self).__init__(args, train)
    ...
    def _scan(self):
        ...
        for i in range(idx_begin + 1, idx_end + 1):
            ...
            list_input.append(os.path.join(self.dir_input,'X{}/{}_LR_{}{}'.format(self.scale, filename, self.scale, self.ext)))
        return list_tar, list_input

    def _set_filesystem(self, dir_data):
        self.apath = dir_data + '/DIV2K'
        self.dir_tar = os.path.join(self.apath, 'DIV2K_train_HR')
        self.dir_input = os.path.join(self.apath, 'DIV2K_train_LR')
        self.ext = '.png'
    ...
    def _name_inputbin(self):
        return os.path.join(
            self.apath,
            'bin',
            '{}_bin_LR_X{}.npy'.format(self.split, self.scale)
        )
