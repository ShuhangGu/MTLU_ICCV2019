...
def make_optimizer(args, target):
    ...
    for name,param in target.named_parameters():
        if param.requires_grad:
            if 'MTLU' in name:
                para_group.append({'params': param, 'weight_decay': 0})
            elif 'PLF' in name:
                para_group.append({'params': param, 'weight_decay': 0})
            else:
                para_group.append({'params':param})
    ...
    return  optimizer_function(para_group, **kwargs_optimizer)
    #optimizer = CustomOptimizer(trainable, **kwargs)
